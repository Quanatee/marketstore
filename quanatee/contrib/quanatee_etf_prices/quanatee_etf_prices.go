        ...
        if ( calendar.IsWorkday(dt.UTC()) && 
           (( int(dt.UTC().Weekday()) == 1 && dt.UTC().Hour() >= 13 ) || 
            ...
            if dt.UTC().Unix() > last.UTC().Unix() && dt.UTC().Unix() >= from.UTC().Unix() && dt.UTC().Unix() <= to.UTC().Unix() {
                ...
                if etfData.PriceData[bar].Open != 0 && etfData.PriceData[bar].High != 0 && etfData.PriceData[bar].Low != 0 && etfData.PriceData[bar].Close != 0 {
                    endOfSlice = bar
...
func GetTiingoPrices(symbol string, from, to, last time.Time, realTime bool, period *utils.Timeframe, calendar *cal.Calendar, token string) (Quote, error) {
	...
	for bar := 0; bar < numrows; bar++ {
        ...
        if dt.UTC().Unix() > last.UTC().Unix() && dt.UTC().Unix() >= from.UTC().Unix() && dt.UTC().Unix() <= to.UTC().Unix() {
            ...
            if iexData[bar].Open != 0 && iexData[bar].High != 0 && iexData[bar].Low != 0 && iexData[bar].Close != 0 {
                endOfSlice = bar
                quote.Epoch[bar] = dt.UTC().Unix()
                ...
                quote.HLC[bar] = (iexData[bar].High + iexData[bar].Low + iexData[bar].Close)/3
            }
