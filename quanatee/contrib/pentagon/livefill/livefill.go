...
func Bars(symbol, marketType string, from, to time.Time) (err error) {
	...
	log.Info("livefill.Bars(%s) ohlcv1(%v) ohlcv2(%v), ohlcv(%v) ohlcv2[0](%v) ohlcv2[-1](%v)", symbol, len(ohlcv.Epoch), len(ohlcv2.Epoch), ohlcv.Epoch[0], ohlcv2.Epoch[0], ohlcv2.Epoch[-1])
